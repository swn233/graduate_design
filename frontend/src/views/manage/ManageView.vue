...
import {
  ...
  Files as IconFiles, // Assuming Files icon for Datasets, or choose another
  Fold as IconFold,
  Expand as IconExpand,
  ArrowDown as IconArrowDown,
  Search as IconSearch,
  SwitchButton as IconSwitchButton,
  DataAnalysis as IconDataAnalysis // Using DataAnalysis for Models, Setting for general settings
...
const activeIndex = ref('user-manage'); // Default to user management
...
onMounted(() => {
  ...
  // Ensure only admin can see this page, or adjust role display as needed
  userInfo.value.role = role === 'admin' ? '管理员' : '用户'; 
  if (role !== 'admin') {
    // Optionally redirect if not admin, or disable features
    // For now, just set role, actual access control might be in router or component logic
    console.warn('Non-admin accessing management view');
  }
});
</script>
      ...
      <el-main style="margin: 0px;padding: 0px;height: 100vh;">
        <keep-alive>
          <transition name="fade" mode="out-in">
            <user-management v-if="activeIndex === 'user-manage'" :is-collapse="isCollapse" />
            <article-management v-else-if="activeIndex === 'article-manage'" :is-collapse="isCollapse" />
            <model-management v-else-if="activeIndex === 'model-manage'" :is-collapse="isCollapse" />
            <dataset-management v-else-if="activeIndex === 'dataset-manage'" :is-collapse="isCollapse" />
          </transition>
        </keep-alive>
      </el-main>
  </el-container>
